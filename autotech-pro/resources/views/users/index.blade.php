...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('users.create') }}" class="btn btn-marca d-inline-flex align-items-center gap-2 px-3 shadow-sm">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        @switch($user->role)
                            @case('manager') <span class="badge badge-manager">Gerente</span> @break
                            @case('attendant') <span class="badge badge-attendant">Atendente</span> @break
                            @case('mechanic') <span class="badge badge-mechanic">Mecânico</span> @break
                            @case('customer') <span class="badge badge-customer">Cliente</span> @break
                        @endswitch
                    </td>
                    ...
                    <td class="pe-4 text-end">
                        <div class="d-flex justify-content-end gap-1">
                            <a href="{{ route('users.edit', $user->id) }}" class="btn btn-sm btn-outline-light border-0" title="Editar">
                                <i data-lucide="user-cog" class="size-4"></i>
                            </a>

                            @if($user->id != session('user_id'))
                            <form method="POST" action="{{ route('users.destroy', $user->id) }}" class="d-inline" onsubmit="return confirm('Deseja remover este acesso?')">
                                @csrf
